...
import { Download, Star, Lock, AlertCircle, Bell } from "lucide-react"
import Link from "next/link"
import { motion } from "framer-motion"

export default function ValorantCheats() {
  // Single Valorant cheat
  const cheat = {
    id: 1,
    title: "Valorant || All-in-One-File",
    game: "Valorant",
    description: "All in 1 download pack, only has 1 file at the moment, will be updated with more cheats later on..",
    features: ["Aimbot", "ESP", "Radar", "Recoil Control"],
    popular: true,
    downloadUrl:
      "https://github.com/iloveskiddingcodeaha/Purpleware/raw/refs/heads/main/Val%20cheat%20-%20purpleware.zip",
    password: "Nothing!",
  }
    ...
    <div className="space-y-12 relative">
      ...
      >
        ...
        <p className="text-lg text-purple-200 max-w-3xl mx-auto text-center">
          Dominate Valorant with our advanced, undetectable cheats. Gain the competitive edge with our aimbot, ESP, and
          other features.
        </p>
      </motion.div>
      ...
      >
        <motion.div whileHover={{ y: -5, transition: { duration: 0.2 } }}>
          <Card className="overflow-hidden bg-[#0A0518] border-purple-900/40 hover:border-purple-700 transition-all duration-300 hover:shadow-xl hover:shadow-purple-900/20 group">
            ...
            <CardHeader>
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <CardTitle className="text-2xl text-purple-100 group-hover:text-white transition-colors">
                    {cheat.title}
                  </CardTitle>
                </div>
              </div>
            ...
            <CardContent className="space-y-4">
              <p className="text-lg text-purple-200 group-hover:text-purple-100 transition-colors">
                {cheat.description}
              </p>
            ...
            <CardFooter className="flex justify-between">
              <Link href={cheat.downloadUrl} className="w-full" target="_blank" rel="noopener noreferrer">
                <Button className="w-full bg-purple-800 hover:bg-purple-700 relative overflow-hidden group py-6 text-lg">
                  <motion.span
                  ...
                  <span className="relative z-10 flex items-center">
                    Download <Download className="ml-2 h-5 w-5 group-hover:translate-y-1 transition-transform" />
                  </span>
                </Button>
              </Link>
            </CardFooter>
      ...
      >
        <h2 className="text-xl font-bold text-purple-100 mb-4">How to Use Our Valorant Cheats</h2>
        <ol className="space-y-3 text-purple-200">
          ...
          >
            ...
            <span>Launch Valorant and enjoy your enhanced gaming experience</span>
          </motion.li>
        </ol>
        <motion.div
      ...
      {/* Important Notice */}
      <motion.div
        className="bg-purple-900/20 border border-purple-900/40 rounded-xl p-6"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.8, duration: 0.8 }}
      >
        <div className="flex items-start gap-4">
          <div className="bg-purple-900/50 p-3 rounded-full flex-shrink-0">
            <AlertCircle className="h-6 w-6 text-purple-200" />
          </div>
          <div>
            <h3 className="text-lg font-medium text-purple-200 mb-2">Important Notice</h3>
            <p className="text-purple-300">
              Our Valorant cheats are regularly updated to stay ahead of anti-cheat systems. For the latest updates and
              announcements, join our Discord community. We recommend using our cheats on alternate accounts to minimize
              risk.
            </p>
            <div className="mt-4">
              <Link href="/discord">
                <Button size="sm" className="bg-purple-900 hover:bg-purple-800 text-white flex items-center gap-2">
                  <Bell className="h-4 w-4" /> Join Discord for Updates
                </Button>
              </Link>
            </div>
          </div>
